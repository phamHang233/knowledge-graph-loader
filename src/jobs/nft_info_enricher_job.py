...
class NFTInfoEnricherJob(BaseJob):
    ...
    def prepare_enrich(self, cursor_batch):
        ...
        contract_ = _w3.eth.contract(Web3.to_checksum_address(w3_multicall.address), abi=MULTICALL_ABI)
        inputs = w3_multicall.get_params()
        response = contract_.functions.aggregate(*inputs).call(block_identifier=self.before_30_days_block)
    ...
    def calculate_apr(self, nft, current_data_response, before_data_response):
        ...
        nft.cal_apr_in_month(self.before_30_days_block, token0_reward_before,
