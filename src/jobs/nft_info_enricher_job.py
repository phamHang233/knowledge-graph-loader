...
class NFTInfoEnricherJob:
    ...
    def _execute(self):
        for batch_idx in range(self.number_of_nfts_batch):
            self._execute_batch(batch_idx)

    def _execute_batch(self, batch_idx):
        try:
            start_time = time.time()
            batch_cursor = self._klg_db.get_nfts_by_flag(_filter={"liquidity": {"$gt": 0}, "flagged": batch_idx})
            # batch_cursor = self._klg_db.get_nfts_by_flag(_filter={"tokenId": "681749"})
            batch_cursor = self._klg_db.get_nfts_by_flag(_filter={"tokenId": "681749"})
            batch_cursor = list(batch_cursor)
            current_pools_info = {}
            data_response, pools_in_batch = self.prepare_enrich(batch_cursor, current_pools_info)
            ...
            updated_nfts = self.enrich_data(batch_cursor, data_response, current_pools_info)
    ...
    def prepare_enrich(self, cursor_batch, current_pools_info):
        list_rpc_call = []
        list_call_id = []
        pools_in_batch = []
        start_time = time.time()
        ...
        self.state_querier.get_batch_nft_fee_with_block_number(
            nfts=current_nfts, pools=current_pools_info, block_number=self.end_block, list_rpc_call=list_rpc_call,
            list_call_id=list_call_id, latest=True)
        self.state_querier.get_batch_nft_fee_with_block_number(
            nfts=current_nfts, pools=current_pools_info, list_rpc_call=list_rpc_call, list_call_id=list_call_id,
            block_number=self.before_30_days_block, latest=False)
    ...
    def enrich_data(self, batch_cursor, data_response, current_pools_info) -> Dict[str, NFT]:
        ...
        for doc in batch_cursor:
            try:
                ...
                if unchanged_nft:
                    ...
                    if not current_pools_info.get(pool_address, {}):
                        fee_growth_global_0 = data_response.get(
                            f'feeGrowthGlobal0X128_{pool_address}_{self.end_block}'.lower())
                        fee_growth_global_1 = data_response.get(
                            f'feeGrowthGlobal1X128_{pool_address}_{self.end_block}'.lower())
                        ...
                        current_pools_info[pool_address] = {
                    ...
                    fee_growth_global_0 = current_pools_info[pool_address]['feeGrowthGlobal0X128']
                    fee_growth_global_1 = current_pools_info[pool_address]['feeGrowthGlobal1X128']
                    fee_growth_global_0_before = current_pools_info[pool_address]['feeGrowthGlobal0X128Before']
                    fee_growth_global_1_before = current_pools_info[pool_address]['feeGrowthGlobal1X128Before']
                    tick_before = current_pools_info[pool_address]['tick']
                    ...
                    fee_growth_low_x128 = data_response.get(
                        f'ticks_{pool_address}_{tick_lower}_{self.end_block}'.lower())
                    fee_growth_hi_x128 = data_response.get(
                        f'ticks_{pool_address}_{tick_upper}_{self.end_block}'.lower())
                    positions = data_response.get(
                        f'positions_{nft_manager_contract}_{token_id}_{self.end_block}'.lower())
