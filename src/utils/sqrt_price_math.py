...
def get_fees(fee_growth_global_0, fee_growth_global_1, fee_growth_0_low, fee_growth_0_hi, fee_growth_inside_0,
             fee_growth_1_low, fee_growth_1_hi, fee_growth_inside_1, liquidity, tick_lower,
    ...
    return uncollect_fee_0, uncollect_fee_1
...
def get_token_amount_of_user(liquidity, sqrt_price_x96, tick, tick_lower, tick_upper, decimals0, decimals1):
    amount0, amount1 = 0, 0
    if liquidity != 0:
        if tick < tick_lower:
...
def convert_price_to_tick(price, decimals0, decimals1):
    tick = math.log(float(price) / 10 ** (decimals1 - decimals0), 1.0001)
    return -int(tick)
